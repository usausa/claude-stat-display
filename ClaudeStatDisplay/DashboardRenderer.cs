...
internal static class DashboardRenderer
{
    ...
    private static void DrawLeftPanel(SKCanvas canvas, DisplayState state)
    {
        ...
        // CONTEXT WINDOW
        var ctxSize = ClaudeContextWindowSize.Resolve(state.Model);
    ...
    private static void DrawRightPanel(SKCanvas canvas, DisplayState state)
    {
        ...
        float y = HeaderH + PanelPadTop + ModelSectionH;
    ...
    private static void DrawRateRow(SKCanvas canvas, string label, double? utilization, string? status, string? resetTime, float x, ref float y, float panelW)
    {
